...
public class ViewCriminalForm extends javax.swing.JFrame {
    ...
    public ViewCriminalForm(String loggedInUsername) 
    {
       ...
       // Initialize the tableModel with the correct column names
    tableModel = new DefaultTableModel(
        new Object[][] {},
        new String[] {"Criminal ID", "Criminal Name", "No. of Crimes", "Address", "NIC Number", "Arrest Date", "Time of Crime", "Description", "View Details"});
    ...
    {
        SearchResultsTable.getColumnModel().getColumn(0).setPreferredWidth(80);  // FIR ID
    SearchResultsTable.getColumnModel().getColumn(2).setPreferredWidth(150); // Contact
    SearchResultsTable.getColumnModel().getColumn(4).setPreferredWidth(200); // NIC Number
    SearchResultsTable.getColumnModel().getColumn(8).setPreferredWidth(100); // View Details (button)
    ...
    try {
        ...
        List<Criminal> criminal = csvHandler.getAllCriminals();
    ...
    private void searchbartxtKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchbartxtKeyReleased
        ...
        else
        {
            ...
            {
                CSVHandler csvHandler = new CSVHandler();
                List<Criminal> criminal = csvHandler.searchCriminals(searchbartxt.getText());
                for (Criminal cri : criminal)
                {
                    tableModel.addRow(new Object[]
                        {
                            cri.getCriminalId(), cri.getComplainantName(), cri.getStatus(),
                        cri.getContact(), cri.getAddress(), cri.getNicNumber(), cri.getdateofBirth(),
                        cri.getTime(), cri.getDescription(), "View Details"
                        });
                    }
                }
                catch (IOException e)
