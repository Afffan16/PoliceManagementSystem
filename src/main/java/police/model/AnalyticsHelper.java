    ...
    public static void showChart(JPanel chartPanelContainer, int selectedMonth, int selectedYear, String filePath) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length < 10) continue; // must have at least 10 columns

                String dateStr = parts[4].replace("\"", "").trim();  // remove quotes if any
                String status = parts[9].replace("\"", "").trim().toLowerCase();// 10th column (index 9)
